...
I can also show the cutoff value in the context of all distances:
...
levelplot(raster::stack(abs(TempHetTE)^(1/5)), # level plot only works with raster files
          scales = list(draw=FALSE), # To remove the Latlong
          main=list("Absolute Temporal gradient [% per 100 yrs]\n(Estimated using five aproaches - Scaled as $X^(1/5)$ for ease of visualization)",side=1,line=-0.5), # Main title
          col.regions = rev(hcl.colors(100,"RdYlBu")), # set the colors
          colorkey = list(col=rev(hcl.colors(100, palette = "RdYlBu"))) # add a legend
          ) +
# Add the country outlines
...
levelplot(raster::stack((SpaceHetByGF)^(1/5)), # level plot only works with raster files
          scales = list(draw=FALSE), # To remove the Latlong
          main=list("Spatial gradient [(% per km)^(1/5)]",side=1,line=-0.5), # Main title
          col.regions = rev(hcl.colors(100,"RdYlBu")), # set the colors
          colorkey = list(col=rev(hcl.colors(100, palette = "RdYlBu"))) # add a legend
          ) +
# Add the country outlines
...
Now with the metrics of velocity and bearings I can then estimate displacement and divergence of climatic vectors (cf. [@ordonezMappingClimaticMechanisms2016]).
...
Divergence of climatic vectors is the standard deviations of bearings as in [@burkeDifferingClimaticMechanisms2019]. This metric indicates how "variable" are the directions of velocity vectors in a given location for the 14 evaluated growth forms.
