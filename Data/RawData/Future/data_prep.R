...
for (i in 1:length(f)) {
  ...
  for(k in 1:ncol(pm)){
    ...
    if(sum(is.na(myvals)) > 0) {
      ...
      resample(myr, r.low, filename = paste("suitability_", dimnames(pm)[[2]][k], "_", scen, ".tif", sep = ""), overwrite = TRUE)
    } else {
      myr <- rasterize(grid025.list[[1]][, 1:2], r.high, field = myvals)
      resample(myr, r.low, filename = paste("suitability_", dimnames(pm)[[2]][k], "_", scen, ".tif", sep = ""), overwrite = TRUE)
    }
...
for(r in 1:length(rcps)){
  f <- list.files(pattern = rcps[r], full.names = TRUE)
  for(i in 1:ncol(pm)){
    gf <- dimnames(pm)[[2]][i]
    ...
    if(sum(is.na(gfs.median)) > 0) {
      ...
      myr[Which(myr == -1, cells = T) ] <- NA
      resample(myr, r.low, filename = paste("suitability_", dimnames(pm)[[2]][i], "_MEDIAN_", rcps[r], ".tif", sep = ""), overwrite = TRUE)
    } else {
      myr <- rasterize(grid025.list[[1]][, 1:2], r.high, field = gfs.median)
      resample(myr, r.low, filename = paste("suitability_", dimnames(pm)[[2]][i], "_MEDIAN_", rcps[r], ".tif", sep = ""), overwrite = TRUE)
    }
