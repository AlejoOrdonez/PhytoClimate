...
The novelty metrics in [@ordonezMappingClimaticMechanisms2016] focus on measuring three different mechanisms by which ecological novelty might emerge:
...
After this, I load the suitability per-growth form under past environmental conditions (@fig-Maps21kaBP), here defined as the Last Glacial Maximum (21kaBP) values:

```{r}
#| label: fig-Maps21kaBP
#| fig-cap: "*Past (21kaBP) Suitability per-growth form*"
...
Given that the data I have for growth-form suitability is the proportion of species within a group for which the evaluated cell has suitable conditions (similar to proportion of species) I will use the Min Chord distance as suggested by [@simpsonAnalogueMethodsPalaeoecology2007], [@overpeckQuantitativeInterpretationFossil1985], and [@gavinStatisticalApproachEvaluating2003].
...
With all the pairwise distances estimated, a technique used to estimate site novelty (as in [@williamsProjectedDistributionsNovel2007], [@williamsNovelClimatesNoanalog2007], [@ordonezCombinedSpeedsClimate2014], and [@ordonezMappingClimaticMechanisms2016]) is to retain the minimum dissimilarity value of the contrast between the target assemblage (here *1950's sites*) and all sites in the reference period (here 21kaBP). This technique is similar to the analogue matching approach in paleoecology ([@overpeckQuantitativeInterpretationFossil1985] and [@flowerMatchingDiatomAssemblages1997]).

Now using the Min Chord distance (as implemented in `anaolgue`), I estimate (using a parallel computing approach) the novelty of each cell in `Maps0kaBP` [the present growth from suitability measured based on 1950 climates, @fig-CordDistEst] when compared to the Last Glacial Maximum growth from suitability as presented in `Maps21kaBP`:

```{r}
#| label: fig-CordDistEst
#| fig-cap: "*Min Chord distance for each current cell in Maps0kaBP to all past cells in Maps21kaBP.*"
...
I also do this estimation using a Chi-Squared distance (@fig-ChiDistEst):

```{r}
#| label: fig-ChiDistEst
#| fig-cap: "*Min Chi-Sqred distance for each current cell in Maps0kaBP to all past cells in Maps21kaBP.*"
...
Now, using this value, the `CordDistRast` object, which contains the dissimilarity values could be masked to indicate which of the current areas are novel when compared to past conditions (@fig-CordDistMapMcarloCutoff1).


```{r}
#| label: fig-CordDistMapMcarloCutoff1
#| fig-cap: "*Areas where NOVEL growth form compositional assemblages are expected based on a cord-distance/ROC-defined criteria.*"
...
If you would like to see the cord-distance/ROC-defined cutoff criteria in the context of all minimum cord-distances you can plot the histogram of distances (@fig-CordDistMapMcarloCutoff2):

```{r}
#| label: fig-CordDistMapMcarloCutoff2
#| fig-cap: "*Histogram showing the distibution of distances and the cord-distance/ROC-defined cutoff.*"
...
The same can now be done for the novelty estimated using the chi-squared distance. Below is the visualization for a ChiSrq-distance/MonteCarlo-defined cut-off criteria (@fig-ChiSqDmcarlothresh1):

```{r}
#| label: fig-ChiSqDmcarlothresh1
#| fig-cap: "*Areas where NOVEL growth form compositional assemblages are expected based on a ChiSrq-distance/MonteCarlo-defined criteria.*"
...
Again, if you would like to see the ChiSrq-distance/MonteCarlo-defined cutoff criteria in the context of all minimum ChiSrq-distances you can plot the histogram of distances (@fig-ChiSqDmcarlothresh2):

```{r}
#| label: fig-ChiSqDmcarlothresh2
#| fig-cap: "*Histogram showing the distibution of distances and the ChiSrq-distance/MonteCarlo-defined cutoff.*"
...
Now, like with the Monte Carlo approach, I can classify the `CordDistRast` object as areas that are(are) novel when compared to past conditions (@fig-CordDMapROCCutoff1).

```{r}
#| label: fig-CordDMapROCCutoff1
#| fig-cap: "*Areas where NOVEL growth form compositional assemblages are expected based on a cord-distance/ROC-defined criteria.*"
...
I can also show the cutoff value in the context of all distances (@fig-CordDMapROCCutoff2).

```{r}
#| label: fig-CordDMapROCCutoff2
#| fig-cap: "*Histogram showing the distibution of distances and the cord-distance/ROC-defined cutoff.*"
# Histogram of dissimilarities representing the distribution of dissimilarity distances and the cut-off values
hist(CordDistRast)
# cut-off value
...
Now, the same procedure is done using a Chi-Squared distance (@fig-ChiSqDROCthreshold1).

```{r}
#| label: fig-ChiSqDROCthreshold1
#| fig-cap: "*Areas where NOVEL growth form compositional assemblages are expected based on a ChiSqr-distance/ROC-defined criteria.*"
...
Now, the same procedure is done using a cord-distance (@fig-CordDROCthreshold1).

```{r}
#| label: fig-CordDROCthreshold1
#| fig-cap: "*Histogram showing the distibution of distances and the cord-distance/ROC-defined cutoff.*"
...
This mechanism focuses on measuring how *fast* would the "suitability" surface of for a given taxa would move in space - assuming that taxa would move from an area of low to an area of higher suitability between two times points. Estimations flow the implementation in [@ordonezCombinedSpeedsClimate2014] and [@ordonezMappingClimaticMechanisms2016].
...
Here, I apply this approach to each growth form suitability maps rather than to a single climate variable (like in [@serra-diazBioclimaticVelocityPace2014]).
...
1)  As the *slope of a generalized least squares regression* on suitability maps for the 21kaBP to 0kaBP period for each cell with a autocorrelation structure of order one (AR1 model). This is the approach taken in [@dobrowskiClimateVelocityContiguous2013] and [@ordonezMappingClimaticMechanisms2016].

2)  As the *slope of a linear model* on suitability maps for the 21kaBP to 0kaBP period for each cell. This is the approach taken in by [@loarieVelocityClimateChange2009].

3)  The *anomaly between the to end periods* (21kaBP vs. 0kaBP) divided by the time between the start (21kaBP) and end (0kaBP) points. This is the approach take in [@sandelInfluenceLateQuaternary2011].
...
I developed the `TempGradFnc()` function to estimate these using all five models of temporal change. Outputs for these, can be seen [HERE](https://github.com/AlejoOrdonez/PhytoClimate/tree/main/Data/Velocity). For illustration, (@fig-TempGradGF) shows how these can be estimated for all growth forms with the *median differences between consecutive time periods* approach.

```{r}
#| label: fig-TempGradGF
#| fig-cap: "*Temporal heterogeneity (%-change per 100yrs) between 21kaBA and 0kaBP*"
...
To get a sense of how these changes compare between methods of evaluating temporal heterogeneity, I now show all five estimates for Evergreen trees (@fig-TempGradTE).

```{r}
#| label: fig-TempGradTE
#| fig-cap: "*Contrast of ABSOLUTE temporal heterogeneity (%-change per 100yrs) metrics for Evergreen trees. Scaled as $X^\frac{1}{5}$ for ease of visualization.*"
...
One of this is estimating a ration when the denominator is zero (a case here when the spatial gradient is zero as all neighboring cells have the same value). In this situation you can either define these cells to have the minimum value of differentiation allowed based on the "accuracy" of your measurements (as done by by [@loarieVelocityClimateChange2009] and [@sandelInfluenceLateQuaternary2011]). An alternative, is to define these areas as the "maximum" velocity used for visualization (1000km per 100yr in this case).
...
With these clarifications, we then estimated the velocity of change for each of the evaluated growth forms (@fig-VelocityGF). But as I also implemented multiple metrics of temporal change, velocity estimates need to be estimated for each of these.

```{r}
#| label: fig-VelocityGF
#| fig-cap: "*Velocity of change for Per Growth Form*"
...
To get a sense of how these changes compare between methods of evaluating temporal heterogeneity, I now show all five estimates for Evergreen trees (@fig-VelocityTE).

```{r}
#| label: fig-VelocityTE
#| fig-cap: "*Velocity of change (km per 100yrs) between 21kaBP and 0kaBP for Evergreen trees.*"
...
I estimated the bearing of the velocity vector using the vector resulting from the sum of the rise and run run of the spatial gradient, reversing the direction of change is the temporal trend was negative (@fig-BearingGF).

```{r}
#| label: fig-BearingGF
#| fig-cap: "*Bearing of the spatial gradients *"
if(length(dir("./Data/Velocity/SpatHet/",
              pattern="All_"))==0){
#Loop over all Growth Forms
  SpaceHetByGF <- lapply(NamesDtFrm$Acro2,
...
```{r}
#| length: fig-DisplacementAll
#| fig-cap: "*Displacement of growth form suitability velocity vectors*"
...
As defined in [@ordonezMappingClimaticMechanisms2016], divergence of climatic vectors is the angle between two velocity vectors. In a multivariate context this definition is not practical, and I instead use the standard deviations of bearings as in [@burkeDifferingClimaticMechanisms2019]. This metric indicates how "variable" are the directions of velocity vectors in a given location for the 14 evaluated growth forms. A low divergence suggests that all velocity vectors of all climate variables would tend to shift along the same axis of direction, whereas high divergences indicate that velocity vectors lack congruence in orientation and hence so might species distribution shifts.

Here, I leverage the velocity estimates for the 14 evaluated growth forms to assess how fast has the *"growth form setup"* changed since the Last Glacial Maxim (LGM; \~21ka) (@fig-DivergenceAllGF). The *Displacement of growth form suitability*. For this, I used the *geometric mean* of the velocity vectors (here implemented as the mean of Log-10 velocities).

```{r}
#| label: fig-DivergenceAllGF
#| fig.cap: "*Divergence of growth form suitability velocity vectors*"
...
Here I will estimate the velocity of change of the growth form suitability surfaces for specific time periods for the Last Termination defined by the GRIP Greenland ice-core oxygen isotope signal as in [@bjorckEventStratigraphyLast1998].
...
***Table 1*** - Depths and preliminary ages (ad1950) of the onset of events and episodes in the GRIP ice-core, including the Holocene epoch. Dates based on [@bjorckEventStratigraphyLast1998].
...
```{r}
#| label: fig-VelocityGS2
#| fig-cap: "*Velocity for all evaluated growth forms during the Greenland Stadial-2 (Post-LGM)*"
...
```{r}
#| label: fig-VelocityGI1
#| fig-cap: "*Velocity for all evaluated growth forms during the Greenland Inerstadial-1 (BA)*"
...
```{r}
#| label: fig-VelocityGS1
#| fig-cap: "*Velocity for all evaluated growth forms during the Greenland Stadial-1 (YD)*"
...
```{r}
#| label: fig-VelocityHolocene
#| fig-cap: "*Velocity for all evaluated growth forms during the Holocene (YD)*"
# plot the Velocity
...
Now with the metrics of velocity and bearings I can then estimate displacement and divergence of climatic vectors (cf. [@ordonezMappingClimaticMechanisms2016]) for mutiple periods (@fig-DisplacementGS2, @fig-DisplacementGI1, @fig-DisplacementGS1, (@fig-DisplacementHolocene).
...
```{r}
#| label: fig-DisplacementGS2
#| fig-cap: "*Displacement of growth form suitability velocity vectors during the GS2**"
(TimePer<-"GS2")
# Load and plot the Velocity magnitudes as a list
...
```{r}
#| label: fig-DisplacementGI1
#| fig-cap: "*Displacement of growth form suitability velocity vectors during the GI1**"
(TimePer<-"GI1")
# Load and plot the Velocity magnitudes as a list
  DisplacementAllList <- lapply(c("AbsDif", "Anomaly1", "Anomaly2", "ARM", "lm"),
                           function(Model){#(Model<-"AbsDif")
...
```{r}
#| label: fig-DisplacementGS1
#| fig-cap: "*Displacement of growth form suitability velocity vectors during the GS1**"
(TimePer<-"GS1")
# Load and plot the Velocity magnitudes as a list
  DisplacementAllList <- lapply(c("AbsDif", "Anomaly1", "Anomaly2", "ARM", "lm"),
                           function(Model){#(Model<-"AbsDif")
...
```{r}
#| label: fig-DisplacementHolocene
#| fig-cap: "*Displacement of growth form suitability velocity vectors during the Holocene**"
(TimePer<-"Holocene")
# Load and plot the Velocity magnitudes as a list
  DisplacementAllList <- lapply(c("AbsDif", "Anomaly1", "Anomaly2", "ARM", "lm"),
                           function(Model){#(Model<-"AbsDif")
...
Divergence of climatic vectors (@fig-DivergencePerPeriod) is the standard deviations of bearings as in [@burkeDifferingClimaticMechanisms2019]. This metric indicates how "variable" are the directions of velocity vectors in a given location for the 14 evaluated growth forms.

```{r}
#| labels: fig-DivergencePerPeriod
#| fig-cap: "*Divergence of growth form suitability velocity vectors  during four Post-LGM periods*"
...
## Print the functions used for estimate velocity
