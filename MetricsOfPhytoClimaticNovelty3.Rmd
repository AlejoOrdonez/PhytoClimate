...
```{r Maps0kaBP, fig.cap="**Fig 1**. *Present (1950) suitability per-growth form.*"}
...
```{r CordDistEst, fig.cap="**Fig 3**. *Min Chord distance for each current cell in Maps0kaBP to all past cells in Maps21kaBP.*"}
...
```{r ChiDistEst, fig.cap="**Fig 4**. *Min Chi-Sqred distance for each current cell in Maps0kaBP to all past cells in Maps21kaBP.*"}
...
```{r CordDistMapMcarloCutoff1, fig.cap="**Fig 5a**. *Areas where NOVEL growth form compositional assemblages are expected based on a cord-distance/ROC-defined criteria.*"}
...
```{r CordDistMapMcarloCutoff2, fig.cap="**Fig 5b**. *Histogram showing the distibution of distances and the cord-distance/ROC-defined cutoff.*"} 
...
```{r ChiSqDmcarlothresh1, fig.cap="**Fig 6a**. *Areas where NOVEL growth form compositional assemblages are expected based on a ChiSrq-distance/MonteCarlo-defined criteria.*"}
if(!"CordDmcarlo.rds"%in%dir("./Data/Novelty/")){
# use a Monte Carlo simulation of dissimilarities to define the suitability cut-off
  a<-Sys.time() # Clocks at ~4mins
...
```{r ChiSqDmcarlothresh2, fig.cap="**Fig 6b**. *Histogram showing the distibution of distances and the ChiSrq-distance/MonteCarlo-defined cutoff.*"}
# Histogram of dissimilarities representing the distribution of dissimilarity distances and the cut-off values
hist(ChiDistRast)
# cut-off value
...
```{r CordDMapROCCutoff1, fig.cap="**Fig 7a**. *Areas where NOVEL growth form compositional assemblages are expected based on a cord-distance/ROC-defined criteria.*"}
# defining the suitable cut-off value
...
```{r CordDMapROCCutoff2, fig.cap="**Fig 7b**. *Histogram showing the distibution of distances and the cord-distance/ROC-defined cutoff.*"}
# Histogram of dissimilarities representing the distribution of dissimilarity distances and the cut-off values
hist(CordDistRast)
# cut-off value
...
```{r ChiSqDROCthreshold1, fig.cap="**Fig 8a**. *Areas where NOVEL growth form compositional assemblages are expected based on a ChiSqr-distance/ROC-defined criteria.*"}
...
plot(ChiDistRast > ChiDCutOffVal,
     ...
     xpd=NA)
```
```{r ChiSqDROCthreshold2, fig.cap="**Fig 8b**. *Histogram showing the distibution of distances and the cord-distance/ROC-defined cutoff.*"}
# Histogram of dissimilarities representing the distribution of dissimilarity distances and the cut-off values
hist(ChiDistRast,
...
```{r TempGradEveGrn, fig.cap="**Fig 9**. *Temporal heterogeneity (%-change per 100yrs) for Evergreen trees*"}
...
# Generate a SpatRaster that estimates the temporal trend/significance for each cell using the app function from terra
  ...
  TempHetEvergreen <- app(MapsEvergreen,
                          ...
                          cores=10 # the function is run automatically in parrallel in 10 cores
                          )
  Sys.time()-a
...
```{r SpacGradEveGrn, fig.cap="**Fig 10**. *Spatial heterogeneity (%-change per km) for Evergreen trees*"}
...
```{r BearingEveGrn, fig.cap="**Fig 11**. *Bearing of the spatial gradient for Evergreen trees*"}
...
```{r Velocity, fig.cap="**Fig 12**. *Velocity of change for Evergreen trees*"}
...
VelocityEvGrn[which(VelocityEvGrn[]<0.001)] <- 0

# Make the velocity for flat areas 100km/centennial
VelocityEvGrn[which(SpacHetEvGrnRast[]==0)] <- 101
# Make the velocity for very fast areas (>200km/centennial) km/centennial
VelocityEvGrn[which(VelocityEvGrn[]>100)] <- 101
...
#plot the Velocity
plot(log10(VelocityEvGrn),
     plg = list(title = 'km per 100yrs'),
     main = "Suitability velocity [Evergreen trees]",
     cex.main = 1.5,
     xpd=NA)
```
...
**First**: Estimate the temporal gradients using a generalised least squares regression on suitability maps for the 21kaBP to 0kaBP period for each cell with a autocorrelation structure of order one (AR1 model).

```{r TempGradAllGF, fig.cap="**Fig 11**. *Temporal gradinets for all evaluated growth forms*"}
# We will loop over all GF names
for(GF.Use in NamesDtFrm$Acro2){#GF.Use<- "G_C4"
  if(!paste0(GF.Use,"_TempChng.tif")%in%dir("./Data/Velocity/")){
                   ...
                   fun = function(x){
                     ...
                     # Filters to ensure convergence
                            if(sum(x,na.rm=T)!=0 & # locations with values
                               ...
                               length(unique(x)) > 2){ # more tna just two unique values ove time.
...
# Save the Raster file
    writeRaster(TempHetRast,
                paste0("./Data/Velocity/",GF.Use,"_TempChng.tif"),
              overwrite=TRUE)
...
TempHetAllList <- lapply(NamesDtFrm$Acro2,
                         function(GF.Use){
                           rast(paste0("./Data/Velocity/",GF.Use,"_TempChng.tif"))
                         })
...
par(oma=c(0,0,2,0))
plot(TempHetAll)
mtext("Temporal gradient [% per 100 yrs]",
      font =2,
      cex=1.5,
      outer = TRUE)
```
...
```{r SpaceGradAllGF, fig.cap="**Fig 12a**. *Spatial gradinets for all evaluated growth forms*"}
# We will loop over all GF names
for(GF.Use in NamesDtFrm$Acro2){#GF.Use<- "G_C4"
  if(!paste0(GF.Use,"_SpatHet.tif")%in%dir("./Data/Velocity/")){
...
# Save the Raster file
    writeRaster(SpacHetRast,
                paste0("./Data/Velocity/",GF.Use,"_SpatHet.tif"),
              overwrite=TRUE)
...
SpatHetAllList <- lapply(NamesDtFrm$Acro2,
                         function(GF.Use){
                           rast(paste0("./Data/Velocity/",GF.Use,"_SpatHet.tif"))
                         })
...
par(oma=c(0,0,2,0))
plot(SpatHetAll)
mtext("Spatial gradient [% per km]",
      font =2,
      cex=1.5,
      outer = TRUE)
```
 
```{r BearingAllGF, fig.cap="**Fig 12b*. *Bearing of the spatial gradinets for all evaluated growth forms*"}

# We will loop over all GF names
for(GF.Use in NamesDtFrm$Acro2){#GF.Use<- "G_C4"
  if(!paste0(GF.Use,"_Bearing.tif")%in%dir("./Data/Velocity/")){
# Make suitability raster for the selected GF for time 1
    MapsPer.GF <- rasterize(x = as.matrix(xy), # points as a matrix
                                   y = cr.ea, #  template raster
                                   values = pml[[1]][,GF.Use] # Values to aggregate
...
# Load the temporal trend  
  TempHetRast <- rast(paste0("./Data/Velocity/",GF.Use,"_TempChng.tif"))
# Turn acording to the temporal trend
...
# Save the Raster file
    writeRaster(BearingRast2,
                paste0("./Data/Velocity/",GF.Use,"_Bearing.tif"),
                overwrite=TRUE)
...
BearingAllList <- lapply(NamesDtFrm$Acro2,
                         function(GF.Use){
                           rast(paste0("./Data/Velocity/",GF.Use,"_Bearing.tif"))
                         })
...
par(oma=c(0,0,2,0))
plot(BearingAll)
mtext("Bearing  [Degrees]",
      font =2,
      cex=1.5,
      outer = TRUE)
```

**Tird**: Estimate the lvelocity magnitude (i.e., speed) as the ratio between the temporal and spatial gradienet.

 
```{r Velocity AllGF, fig.cap="**Fig 13*. *Velocity for all evaluated growth forms*"}
...
# Load the Temporal gradients as a list
    TempHet <- rast(paste0("./Data/Velocity/",GF.Use,"_TempChng.tif"))

# Load and plot the Temporal gradients as a list
    SpatHet <- rast(paste0("./Data/Velocity/",GF.Use,"_SpatHet.tif"))
...
# Make the velocity for flat areas 100km/centennial
    Velocity[which(SpatHet[]==0)] <- 101
# Make the velocity for very fast areas (>200km/centennial) km/centennial
    Velocity[which(Velocity[]>1000)] <- 101
# Save the Raster file
...
# plot the Bearing gradient
par(oma=c(0,0,2,0))
plot(log10(VelocityAll))
mtext("log10-Velocity  [km per 100yrs]",
      font =2,
      cex=1.5,
      outer = TRUE)
```
...
1) Add ice sheet covers to the maps
2) Consider the spatial heterogeneity as the Avg of the first 3 periods
3) Temporal gradients as the avg of changes between consecutive 500 yrs.
4) Analog vector trajectory
